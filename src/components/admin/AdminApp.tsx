...
function ApiSettingsView({ token }: { token: string }) {
  ...
  const [cashfree, setCashfree] = useState({ appId: "", secretKey: "", webhookSecret: "", payoutClientId: "", payoutClientSecret: "", liveMode: false, activeGateway: "RAZORPAY", secretKeyConfigured: false, webhookSecretConfigured: false, payoutClientSecretConfigured: false, gatewayReady: false, payoutsReady: false });
  ...
  useEffect(() => {
    api<ApiResponse<typeof cashfree>>("/api/v1/admin/settings/cashfree", token)
      .then((result) => setCashfree((value) => ({ ...value, ...result.data, secretKey: "", webhookSecret: "", payoutClientSecret: "" })))
      .catch((reason) => setError((reason as Error).message));
  }, [token]);
  ...
  async function saveCashfree(event: FormEvent) {
    ...
    try {
      const result = await api<ApiResponse<typeof cashfree>>("/api/v1/admin/settings/cashfree", token, { method: "PATCH", body: JSON.stringify({ appId: cashfree.appId, ...(cashfree.secretKey ? { secretKey: cashfree.secretKey } : {}), payoutClientId: cashfree.payoutClientId, ...(cashfree.payoutClientSecret ? { payoutClientSecret: cashfree.payoutClientSecret } : {}), liveMode: cashfree.liveMode, activeGateway: cashfree.activeGateway }) });
      setCashfree((value) => ({ ...value, ...result.data, secretKey: "", webhookSecret: "", payoutClientSecret: "" })); setMessage(result.message || "Cashfree settings saved.");
    ...
    <>
      ...
      {loading ? (
        ...
        <form className="admin-card api-settings-card razorpay-settings-card" onSubmit={saveRazorpay}>
          ...
          <div className="razorpay-readiness"><span className={razorpay.gatewayReady ? "ready" : ""}><Check/> Receive payments: {razorpay.gatewayReady ? "ready" : "needs configuration"}</span><span className={razorpay.payoutsReady ? "ready" : ""}><Check/> Owner payouts: {razorpay.payoutsReady ? "ready" : "needs source account"}</span><span className={cashfree.activeGateway === "RAZORPAY" ? "ready" : ""}><Check/> {cashfree.activeGateway === "RAZORPAY" ? "Active gateway" : "Standby"}</span></div>
        ...
        <form className="admin-card api-settings-card razorpay-settings-card" onSubmit={saveCashfree}>
          <div className="api-settings-card-heading"><i><Landmark /></i><div><h2>Cashfree Payments & Payouts</h2><p>Receive booking payments through Hosted Checkout and send owner wallet withdrawals through Cashfree Payouts V2.</p></div><StatusBadge value={cashfree.gatewayReady && cashfree.payoutsReady ? "active" : "inactive"} /></div>
          <div className="razorpay-readiness"><span className={cashfree.gatewayReady ? "ready" : ""}><Check/> Receive payments: {cashfree.gatewayReady ? "ready" : "needs PG credentials"}</span><span className={cashfree.payoutsReady ? "ready" : ""}><Check/> Owner payouts: {cashfree.payoutsReady ? "ready" : "needs Payouts credentials"}</span><span className={cashfree.activeGateway === "CASHFREE" ? "ready" : ""}><Check/> {cashfree.activeGateway === "CASHFREE" ? "Active gateway" : "Standby"}</span></div>
          <div className="gateway-field-section"><h3>Receive payments</h3><p>Credentials from Cashfree Payments → Developers → API Keys.</p><div className="razorpay-fields"><label>Payments App ID<input value={cashfree.appId} onChange={(e) => setCashfree({ ...cashfree, appId: e.target.value })} placeholder="Cashfree Payments App ID" autoComplete="off" required /></label><label>Payments secret key<input type="password" value={cashfree.secretKey} onChange={(e) => setCashfree({ ...cashfree, secretKey: e.target.value })} placeholder={cashfree.secretKeyConfigured ? "Configured — enter only to replace" : "Enter Payments secret key"} autoComplete="new-password" required={!cashfree.secretKeyConfigured} /></label></div></div>
          <div className="gateway-field-section"><h3>Owner payouts</h3><p>Use credentials from Cashfree Payouts → Developers → API Keys. These may differ from Payments credentials.</p><div className="razorpay-fields"><label>Payouts Client ID<input value={cashfree.payoutClientId} onChange={(e) => setCashfree({ ...cashfree, payoutClientId: e.target.value })} placeholder="Cashfree Payouts Client ID" autoComplete="off" required /></label><label>Payouts Client secret<input type="password" value={cashfree.payoutClientSecret} onChange={(e) => setCashfree({ ...cashfree, payoutClientSecret: e.target.value })} placeholder={cashfree.payoutClientSecretConfigured ? "Configured — enter only to replace" : "Enter Payouts Client secret"} autoComplete="new-password" required={!cashfree.payoutClientSecretConfigured} /></label></div></div>
          <div className="razorpay-fields"><label>Active gateway for receive and payout<select value={cashfree.activeGateway} onChange={(e) => setCashfree({ ...cashfree, activeGateway: e.target.value })}><option value="RAZORPAY">Razorpay + RazorpayX</option><option value="CASHFREE">Cashfree Payments + Payouts</option></select><small>The backend uses this provider for new booking collections and owner withdrawal requests.</small></label><label className="razorpay-mode"><input type="checkbox" checked={cashfree.liveMode} onChange={(e) => setCashfree({ ...cashfree, liveMode: e.target.checked })}/><span><b>Cashfree production mode</b><small>Keep disabled while testing with sandbox credentials.</small></span></label></div>
