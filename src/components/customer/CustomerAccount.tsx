...
export function CustomerAccount() {
  ...
  const [customer, setCustomer] = useState<Customer | null>(null),
    [view, setView] = useState<View>("profile"),
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiRequest<{ data: Booking[] }>("/api/v1/customer/bookings", current),
  ...
  async function profile(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await apiRequest("/api/v1/customer/me", token, {
        method: "PATCH",
  ...
  async function addTraveller(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const f = new FormData(e.currentTarget);
    try {
      await apiRequest("/api/v1/customer/co-travellers", token, {
        method: "POST",
        body: JSON.stringify(Object.fromEntries(f)),
      });
      e.currentTarget.reset();
      await load();
      flash("Co-traveller added");
    } catch (x) {
      setError((x as Error).message);
    }
  }
  async function removeTraveller(id: string) {
    await apiRequest(`/api/v1/customer/co-travellers/${id}`, token, {
      method: "DELETE",
    });
    await load();
  }
  ...
  const nav: [View, string, React.ReactNode][] = [
    ["profile", "My profile", <UserRound key="p" />],
    ["travellers", "Co-travellers", <Users key="t" />],
    ["trips", "My trips", <CalendarDays key="b" />],
    ["wishlist", "Wishlist", <Heart key="w" />],
    ["devices", "Logged-in devices", <Laptop key="d" />],
    ...
    >
      ...
      <div className="account-hero">
        <div className="container">
          <label
            className={`customer-avatar customer-avatar-upload ${customer.avatarUrl ? "has-photo" : ""}`}
            style={
              customer.avatarUrl
      ...
      <div className="container customer-account-layout">
        <aside>
          <p className="account-nav-label">MY ACCOUNT</p>
          <nav>
          ...
          <div className="customer-secure">
            <ShieldCheck />
            <p>
              <b>Your account is protected</b>WhatsApp-secured customer access.
            </p>
          </div>
        </aside>
        <div className="customer-account-panels">
          {(notice || error) && (
            <p className={error ? "account-message error" : "account-message"}>
              {error || notice}
            </p>
          )}
          {view === "profile" && (
          ...
          {view === "travellers" && (
            <section>
              ...
              <form onSubmit={addTraveller}>
                ...
                <label>
                  Relationship
                  <input name="relation" placeholder="Family, friend…" />
                </label>
                <label>
                  Gender
                  <select name="gender">
                ...
                <button>
                  <Plus /> Add co-traveller
                </button>
              </form>
              <div className="traveller-list">
                {customer.coTravellers?.map((x) => (
                  <article key={x.id}>
                    <div className="mini-avatar">{x.name[0]}</div>
                    <p>
                      <b>{x.name}</b>
                      <span>{x.relation || x.gender || "Traveller"}</span>
                    </p>
                    <button
                      onClick={() => removeTraveller(x.id)}
                ...
                {!customer.coTravellers?.length && (
                  <div className="account-empty">
                    <Users />
                    <h3>No co-travellers saved</h3>
                    <p>Add traveller details to make future bookings faster.</p>
                  </div>
                )}
              </div>
            </section>
          ...
          {view === "trips" && (
            <section>
              ...
              <div className="trip-tabs">
                {["upcoming", "cancelled", "completed"].map((x) => (
                  <button
                    className={tripTab === x ? "active" : ""}
                    onClick={() => setTripTab(x)}
                    key={x}
                  >
                    {x}
                  </button>
              ...
              <div className="trip-list">
                ...
                {!filtered.length && (
                  <div className="account-empty">
                    <CalendarDays />
                    <h3>No {tripTab} bookings</h3>
                    <p>Your matching stays will appear here.</p>
                    <Link href="/hotels">Plan a trip</Link>
                  </div>
                )}
              </div>
            </section>
          ...
          {view === "wishlist" && (
            <section>
              ...
              <div className="account-empty">
                <Heart />
                <h3>Your dream stays, together</h3>
                <p>Open your wishlist to review every property you saved.</p>
          ...
          {view === "devices" && (
            <section>
              ...
              <button
                ...
                onClick={async () => {
                  await apiRequest("/api/v1/customer/sessions", token, {
                    method: "DELETE",
                  });
                  logout();
                }}
              ...
              <form onSubmit={password}>
                ...
                <label>
                  ...
                  <input
                    ...
                    required
                  />
                  <small>Include a letter, number and special character.</small>
                </label>
      ...
      <ConfirmDialog
        ...
        description="Are you sure you want to logout? You’ll need to login again to view your trips, wishlist and profile."
