...
export function Header() {
  ...
  const [open,setOpen]=useState(false),[authOpen,setAuthOpen]=useState(false),[authView,setAuthView]=useState<"login"|"forgot">("login"),[profileOpen,setProfileOpen]=useState(false),[logoutOpen,setLogoutOpen]=useState(false),[customer,setCustomer]=useState<Customer|null>(null);
  ...
  useEffect(()=>{const mode=new URLSearchParams(window.location.search).get("login");if(mode==="1"||mode==="forgot")window.setTimeout(()=>{setAuthView(mode==="forgot"?"forgot":"login");setAuthOpen(true)},0)},[]);
  ...
  return <><header className={`site-header site-header-${variant} sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur`}><div className="container site-header-inner flex h-18 items-center justify-between">
    ...
    <div className="header-actions"><Link href="/wishlist" className="icon-btn" aria-label="Wishlist"><Heart/></Link><Link href="/list-your-property" className="owner-btn hidden lg:inline-flex">List Your Property</Link>
      {!token?<button className="btn-primary header-login-button hidden sm:inline-flex" onClick={()=>{setAuthView("login");setAuthOpen(true)}}>Login or Create Account</button>:<div className="header-profile-wrap"><button className="header-profile-button" onClick={()=>setProfileOpen(!profileOpen)} aria-expanded={profileOpen}><span>{initial}</span><b>{name}</b><ChevronDown/></button>{profileOpen&&<div className="header-profile-menu"><p>You are viewing your personal profile</p><Link href="/account" onClick={()=>setProfileOpen(false)}><UserRound/><span><b>My profile</b><small>Manage account and personal details</small></span></Link><Link href="/wishlist" onClick={()=>setProfileOpen(false)}><Heart/><span><b>Wishlist</b><small>Review your saved stays</small></span></Link><Link href="/account" onClick={()=>setProfileOpen(false)}><CalendarDays/><span><b>My trips</b><small>See your current and past bookings</small></span></Link><button onClick={()=>{setProfileOpen(false);setLogoutOpen(true)}}><LogOut/> Logout</button></div>}</div>}
    </div>
  </div></header>
  {open&&<div className="fixed inset-0 z-[100] bg-slate-950/60 lg:hidden" onClick={()=>setOpen(false)}><aside role="dialog" aria-modal="true" aria-label="Mobile menu" className="h-dvh w-[84%] max-w-sm overflow-y-auto bg-white p-6 shadow-2xl" onClick={event=>event.stopPropagation()}><div className="mb-7 flex items-center justify-between"><Brand/><button className="icon-btn" onClick={()=>setOpen(false)} aria-label="Close menu"><X/></button></div><nav className="grid gap-1">{links.map(link=><Link onClick={()=>setOpen(false)} key={link} href={hrefFor(link)} className="rounded-xl px-4 py-3 font-semibold hover:bg-red-50">{link}</Link>)}<hr className="my-3 border-slate-200"/><Link onClick={()=>setOpen(false)} href="/list-your-property" className="rounded-xl border border-maroon px-4 py-3 font-bold text-maroon">List Your Property</Link><hr className="my-3 border-slate-200"/>{token?<><Link onClick={()=>setOpen(false)} href="/account" className="mobile-profile-link"><span>{initial}</span><b>{name}</b></Link><button className="mobile-logout" onClick={()=>{setOpen(false);setLogoutOpen(true)}}><LogOut/> Logout</button></>:<button onClick={()=>{setOpen(false);setAuthView("login");setAuthOpen(true)}} className="rounded-xl bg-charcoal px-4 py-3 text-center font-bold text-white">Login or Create Account</button>}</nav></aside></div>}
  {authOpen&&<CustomerAuthModal open onClose={closeAuth} initialView={authView} onAuthenticated={data=>setCustomer(data)}/>}<ConfirmDialog open={logoutOpen} title="Logout from your account?" description="Are you sure you want to logout? You’ll need to verify again to access your trips and profile." onCancel={()=>setLogoutOpen(false)} onConfirm={()=>{setLogoutOpen(false);logout()}}/>
  </>;
}
