"use client";
import { CalendarDays, ChevronDown, MapPin, Minus, Plus, Search, Users, X } from "lucide-react";
import { useRouter } from "next/navigation";
import { useEffect, useRef, useState } from "react";

type Panel = "destination" | "dates" | "guests" | null;
const suggestions = ["Goa", "Mumbai", "Manali", "Udaipur", "Jaipur", "Kerala", "Lonavala", "New Delhi"];
...
export function SearchBox(){
  ...
  const [panel,setPanel]=useState<Panel>(null); const [destination,setDestination]=useState("");
  const [checkInDay,setCheckInDay]=useState(24); const [checkOutDay,setCheckOutDay]=useState(27);
  const [adults,setAdults]=useState(2); const [rooms,setRooms]=useState(1);
  ...
  const submit=(event:React.FormEvent)=>{event.preventDefault();const q=new URLSearchParams({destination:destination||"Goa",checkIn:`2026-08-${String(checkInDay).padStart(2,"0")}`,checkOut:`2026-08-${String(checkOutDay).padStart(2,"0")}`,guests:String(adults)});router.push(`/hotels?${q}`)};
  ...
  return <form ref={wrap} onSubmit={submit} className="search-panel">
    ...
    <button type="button" className={`search-field ${panel==="guests"?"active":""}`} onClick={()=>setPanel(panel==="guests"?null:"guests")}><span>Guests & Rooms</span><div><Users/><strong>{adults} Guests, {rooms} Room{rooms>1?"s":""}</strong><ChevronDown/></div></button>
    ...
    {panel&&<div className={`search-popover ${panel}`}>
      ...
      {panel==="guests"&&<div className="guest-list">{[["Guests","Age 13 and above",adults,setAdults],["Rooms","Number of rooms",rooms,setRooms]].map(([title,desc,count,setCount])=><div key={title as string}><span><b>{title as string}</b><small>{desc as string}</small></span><div><button type="button" onClick={()=>{const setter=setCount as React.Dispatch<React.SetStateAction<number>>;setter(Math.max(1,(count as number)-1))}}><Minus/></button><b>{count as number}</b><button type="button" onClick={()=>{const setter=setCount as React.Dispatch<React.SetStateAction<number>>;setter((count as number)+1)}}><Plus/></button></div></div>)}</div>}
    </div>}
  </form>
}
