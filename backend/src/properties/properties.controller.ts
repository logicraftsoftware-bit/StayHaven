...
export class PropertiesController {
  ...
  private assertSite(
    ...
    if (
      ...
      !user.siteIds?.includes(String(siteId))
    )
      throw new ForbiddenException(
        'This property belongs to an unassigned marketplace site',
      );
  }
  @Get() async list(
    @Query() q: PropertyQueryDto,
  ...
  @Get(':id') async get(
    ...
    const data = await this.s.getAdminView(id);
    this.assertSite(r.user, data.siteId);
