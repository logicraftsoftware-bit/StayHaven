import { IsArray, IsEmail, IsEnum, IsMongoId, IsOptional, IsString, IsUrl, MinLength } from 'class-validator';
...
export class CreateManagedAdminDto {
  ...
  @IsArray() @IsEnum(AdminPermission, { each: true }) permissions: AdminPermission[];
...
export class UpdateManagedAdminDto {
  ...
  @IsOptional() @IsArray() @IsEnum(AdminPermission, { each: true }) permissions?: AdminPermission[];
