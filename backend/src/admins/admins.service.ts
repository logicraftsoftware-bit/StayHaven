...
import { AdminPermission, ALL_ADMIN_PERMISSIONS } from '../common/enums/admin-permission.enum';
...
export class AdminsService {
  ...
  private assertCanManage(actor: AdminDocument, permissions: AdminPermission[], siteIds: string[]) {
    if (actor.role !== Role.SUPER_ADMIN && (actor.adminLevel !== 'MAIN_ADMIN' || !actor.permissions?.includes(AdminPermission.MANAGE_USERS)))
    ...
    if (permissions.some((permission) => !allowed.has(permission)))
      throw new ForbiddenException('You cannot grant a permission you do not have');
    if (actor.role !== Role.SUPER_ADMIN) {
      const sites = new Set((actor.siteIds || []).map(String));
      if (siteIds.some((siteId) => !sites.has(siteId)))
        throw new ForbiddenException('You cannot assign a site you do not manage');
    }
  }
  ...
  async listManaged(actorId: string) {
    ...
    if (actor.role !== Role.SUPER_ADMIN && (actor.adminLevel !== 'MAIN_ADMIN' || !actor.permissions?.includes(AdminPermission.MANAGE_USERS)))
      throw new ForbiddenException('User management permission is required');
    const filter = actor.role === Role.SUPER_ADMIN
      ? { _id: { $ne: actor._id } }
      : { createdBy: actor._id };
    return this.model.find(filter).sort({ createdAt: -1 }).populate('siteIds', 'name domain city').exec();
  }
  async listAssignableSites(actorId: string) {
    const actor = await this.actor(actorId);
    if (actor.role !== Role.SUPER_ADMIN && (actor.adminLevel !== 'MAIN_ADMIN' || !actor.permissions?.includes(AdminPermission.MANAGE_USERS)))
      throw new ForbiddenException('User management permission is required');
    return this.sites.list(actor.role === Role.SUPER_ADMIN ? undefined : (actor.siteIds || []).map(String));
  }
  async createManaged(actorId: string, dto: CreateManagedAdminDto) {
    const actor = await this.actor(actorId);
    if (actor.role !== Role.SUPER_ADMIN && dto.adminLevel === 'MAIN_ADMIN')
      throw new ForbiddenException('Only the Super Admin can create a main admin');
    if (dto.adminLevel !== 'MAIN_ADMIN' && dto.permissions.includes(AdminPermission.MANAGE_USERS))
      throw new ForbiddenException('User management can only be assigned to a main admin');
    this.assertCanManage(actor, dto.permissions, dto.siteIds);
    if (await this.model.exists({ email: dto.email.toLowerCase() }))
      throw new BadRequestException('An administrator with this email already exists');
    ...
    await this.audit.record({ actorId: actor._id, actorRole: actor.role, action: 'ADMIN_CREATED', entityType: 'ADMIN', entityId: admin._id });
  ...
  async updateManaged(actorId: string, id: string, dto: UpdateManagedAdminDto) {
    ...
    if (!target || target.role === Role.SUPER_ADMIN) throw new NotFoundException('Managed administrator not found');
    if (actor.role !== Role.SUPER_ADMIN && String(target.createdBy) !== String(actor._id))
      throw new ForbiddenException('You can only manage users you created');
    if (actor.role !== Role.SUPER_ADMIN && dto.adminLevel === 'MAIN_ADMIN')
      throw new ForbiddenException('Only the Super Admin can assign the main admin level');
    const permissions = dto.permissions || (target.permissions as AdminPermission[]) || [];
    const siteIds = dto.siteIds || (target.siteIds || []).map(String);
    if ((dto.adminLevel || target.adminLevel) !== 'MAIN_ADMIN' && permissions.includes(AdminPermission.MANAGE_USERS))
      throw new ForbiddenException('User management can only be assigned to a main admin');
    ...
    if (dto.siteIds !== undefined) target.siteIds = dto.siteIds.map((siteId) => new Types.ObjectId(siteId));
    if (dto.status !== undefined) target.status = dto.status;
    await target.save();
    await this.audit.record({ actorId: actor._id, actorRole: actor.role, action: 'ADMIN_UPDATED', entityType: 'ADMIN', entityId: target._id });
